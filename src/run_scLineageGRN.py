...
# Publication-facing scLineageGRN entry point.
...
# -----------------------------
# scLineageGRN attention and encoder blocks
# -----------------------------
...
class ScLineageGRNEncoder(nn.Module):
...
# -----------------------------
# scLineageGRN decoder
# -----------------------------

class ScLineageGRNDecoder(nn.Module):
...
class ScLineageGRNDecoderNoBilinear(nn.Module):
...
class ScLineageGRNDecoderLinear(nn.Module):
...
def split_indices(n: int, seed: int, ratios=(0.7,0.1,0.2)):
    rng = np.random.RandomState(seed); idx = np.arange(n); rng.shuffle(idx)
    n_tr=int(n*ratios[0]); n_va=int(n*ratios[1]); tr=idx[:n_tr]; va=idx[n_tr:n_tr+n_va]; te=idx[n_tr+n_va:]
    return tr,va,te
...
def split_by_tf(edges_keep: np.ndarray, seed: int, ratios=(0.7, 0.1, 0.2),
    ...
    return split_indices(len(edges_keep), seed=seed, ratios=ratios)
...
def build_parser():
    ...
    # defaults: your best config
    p.add_argument('--expr', default='data/Chan_mouse_embryo/embryo1/preembryo1_all.csv')
    # p.add_argument('--expr', default='data/c.elegans/c.elegans_sample.csv')
    p.add_argument('--ref',  default='assets/networks/refNetwork.csv')
    # p.add_argument('--ref',  default='data/c.elegans/TableS3_WT_functional_priors.csv')
    p.add_argument('--config', default=None)
    p.add_argument('--outdir', default='../results/main/scLineageGRN/')
    # lineage kernels, smoothing, graphs
    p.add_argument('--lineage_k_path', type=str, default='data/Chan_mouse_embryo/embryo1/lineage_K_all.npy')
    # p.add_argument('--lineage_k_path', type=str, default='data/c.elegans/lineage_K.npy')
...
def main():
    ...
    print('[config] using scLineageGRN default configuration')
    set_seed(args.seed)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    if not args.lineage_k_path:
        raise ValueError("lineage_k_path is required for scLineageGRN.")
    ...
    K = torch.from_numpy(np.load(args.lineage_k_path).astype(np.float32)).to(device)
    ...
    if args.split_mode == "tf_holdout":
        ...
        tr_idx, va_idx, te_idx = split_indices(len(y_keep), seed=args.seed)
    ...
    encoder = ScLineageGRNEncoder(in_dim=X_model.size(1), proj_dim=args.proj_dim,
    ...
    if args.linear_decoder:
        decoder = ScLineageGRNDecoderLinear(emb_dim=args.out_dim).to(device)
    elif args.no_bilinear:
        decoder = ScLineageGRNDecoderNoBilinear(emb_dim=args.out_dim, hid=max(128, 2*args.proj_dim),
            dropout=args.dropout).to(device)
    else:
        decoder = ScLineageGRNDecoder(emb_dim=args.out_dim, hid=max(128, 2*args.proj_dim),
            dropout=args.dropout).to(device)

...
# Publication-facing LinTopoGRN entry point.
...
def resolve_output_dir(path: Optional[str]) -> str:
    if not path:
        return str(PROJECT_ROOT / ".." / "results" / "main" / "LinTopoGRN")
    p = os.path.expanduser(path)
    if os.path.isabs(p):
        return p
...
# -----------------------------
# LinTopoGRN attention and encoder blocks
# -----------------------------
...
class LinTopoGRNEncoder(nn.Module):
...
class LinTopoGRNDecoder(nn.Module):
...
class LinTopoGRNDecoderNoBilinear(nn.Module):
...
class LinTopoGRNDecoderLinear(nn.Module):
...
def build_parser():
    ...
    p.add_argument('--outdir', default='../results/main/LinTopoGRN/')
...
def main():
    ...
    print('[config] using LinTopoGRN default configuration')
    set_seed(args.seed)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    if not args.lineage_k_path:
        raise ValueError("lineage_k_path is required for LinTopoGRN.")
    ...
    encoder = LinTopoGRNEncoder(in_dim=X_model.size(1), proj_dim=args.proj_dim,
    ...
    if args.linear_decoder:
        decoder = LinTopoGRNDecoderLinear(emb_dim=args.out_dim).to(device)
    elif args.no_bilinear:
        decoder = LinTopoGRNDecoderNoBilinear(emb_dim=args.out_dim, hid=max(128, 2*args.proj_dim),
            dropout=args.dropout).to(device)
    else:
        decoder = LinTopoGRNDecoder(emb_dim=args.out_dim, hid=max(128, 2*args.proj_dim),
            dropout=args.dropout).to(device)
